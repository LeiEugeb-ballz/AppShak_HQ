...
ui_out = (root / 'ui.out.log').open('w', encoding='utf-8')
ui_err = (root / 'ui.err.log').open('w', encoding='utf-8')
...
try:
    ...
    procs.append(subprocess.Popen(['cmd.exe', '/c', 'npm run dev -- --host 127.0.0.1 --port 5173'], cwd='appshak-ui', stdout=ui_out, stderr=ui_err))

    ready = False
    for _ in range(60):
        time.sleep(1)
        try:
            s1, _ = fetch('http://127.0.0.1:5173/')
            s2, b2 = fetch('http://127.0.0.1:8010/api/health')
            payload = json.loads(b2.decode('utf-8'))
            if s1 == 200 and s2 == 200 and payload.get('status') == 'ok':
                ready = True
                break
        except Exception:
            pass
    ...
    for p in procs:
        if p.poll() is None:
            p.kill()
    for h in [proj_out, proj_err, obs_out, obs_err, ui_out, ui_err]:
        h.close()
